...
export class CompendiumBrowserAbilitiesTab extends CompendiumBrowserTab {
  constructor(browser) {
    super(browser),
      ...
      (this.templatePath = "systems/shaanrenaissance/templates/compendium-browser/partials/abilities.hbs"),
      (this.searchFields = ["name"]),
      (this.storeFields = ["type", "name", "img", "uuid", "source"]),
      (this.index = ["img", "system.pouvoirType.value", "system.pouvoir.value", "system.rang.value", "system.source.value"]),
  ...
  async loadData() {
    ...
    (this.indexData = abilities),
      (this.filterData.checkboxes.domains.options = this.generateCheckboxOptions(CONFIG.shaanRenaissance.SRdomains)),
      (this.filterData.checkboxes.types.options = this.generateCheckboxOptions(CONFIG.shaanRenaissance.abilitiesTypes)),
      (this.filterData.checkboxes.rangs.options = this.generateCheckboxOptions(CONFIG.shaanRenaissance.pouvoirs.pouvoirRank)),
      (this.filterData.checkboxes.source.options = this.generateSourceCheckboxOptions(sources)),
      console.debug("SHAAN RENAISSANCE | Compendium Browser | Finished loading abilities");
  }
  // A changer
  filterIndexData(entry) {
    ...
    return (
      !(checkboxes.domains.selected.length && !checkboxes.domains.selected.includes(entry.domain)) &&
      !(checkboxes.types.selected.length && !checkboxes.types.selected.includes(entry.type)) &&
      !(checkboxes.rangs.selected.length && !checkboxes.rangs.selected.includes(entry.rang)) &&
